...
export default function NavMenu({ currentPath = 'home' }: NavMenuProps) {
    ...
    return (
        <nav className="w-full flex flex-col lg:flex-row items-center px-5 py-5 justify-between">
            <Link href={'/'}>
                <Image
                    src={DevFestLogo}
                    alt={""}
            ...
            <div className="
                    ...
                    {menuLinks.map((link) => {
                        return (
                            <li key={link.path} className={
                                clsx({
                                    'text-devfest-markee border-b-2 border-devfest-markee': currentPath.toLowerCase() === link.title.toLowerCase(), 
                                })
                            }>
                ...
                <Button borderRadius={50} px={10} py={8} display={{ base: 'none', lg: 'flex' }}>
